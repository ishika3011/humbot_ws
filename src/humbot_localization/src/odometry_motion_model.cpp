#include <humbot_localization/odometry_motion_model.hpp>
    ...
    
{
    ...
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>("humbot_controller/odom", 10, std::bind(&OdometryMotionModel::odomCallback, this, _1));
...
void OdometryMotionModel::odomCallback(const nav_msgs::msg::Odometry &odom)
{
    ...
    {
        delta_rot1 = angle_diff(atan2(odom_y_increment, odom_x_increment), yaw);   //here
    }
    double delta_trans = sqrt(std::pow(odom_x_increment, 2) + std::pow(odom_y_increment, 2));
