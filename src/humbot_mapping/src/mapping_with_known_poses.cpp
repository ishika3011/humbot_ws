...
#include "humbot_mapping/mapping_with_known_poses.hpp"
...
void MappingWithKnownPoses::scanCallback(const sensor_msgs::msg::LaserScan &scan)
{
    ...
    {
        RCLCPP_ERROR(get_logger(), "Unable to transform between %s and %s", map_.header.frame_id.c_str(), scan.header.frame_id.c_str());
        return;
    }
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<humbot_mapping::MappingWithKnownPoses>("mapping_with_known_poses");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}